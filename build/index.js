...
class LMap {
  ...
  new_map($el) {
    ...
    L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
      ...
      tileSize: 512,
      zoomOffset: -1,
      accessToken: 'pk.eyJ1Ijoidml0b3JqYWd1YXQiLCJhIjoiY2w4YzNhbW11MDMwMjN2bnRtb3I3d3VsZCJ9.puMAG2x0aRCow4aQmACz7A'
    }).addTo(map);
    map.markers = [];
    var that = this;
...
class Search {
  ...
  keyPressDispatcher(e) {
    const key = e.keyCode;
    if (key == 83 && !this.isOverlayOpen && !jquery__WEBPACK_IMPORTED_MODULE_0___default()('input, textarea').is(':focus') //only if the 's' key is not being pressed inside an input or textarea
    ) {
      this.openOverlay();
    }
